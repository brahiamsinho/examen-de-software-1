...
## 2026-09-22 — Import a diagram from an image, via OpenAI vision (DD183, delegated build, no SDD)

- **DD183** New `POST /{doc_id}/image-command`: the user uploads a photo/screenshot of a UML class diagram (whiteboard sketch, another tool's export, ...) and it gets replicated on the current canvas, via `gpt-4o-mini`'s vision input. This reuses ~90% of DD180's voice-command feature as-is: the same 9-command tool schema (`tools.py`), the same name→id `Translator`, the same `command_from_payload`/`submit_command` pipeline, the same partial-failure policy (each element applies independently — extracted into a shared `_apply_translated_commands` helper now used by both `apply_voice_command` and `apply_image_command`), and the same `VoiceCommandResult`/`AppliedCommand` response shape. The only genuinely new pieces: `OpenAiClient.generate_function_calls_from_image` (multimodal Chat Completions, a base64 `data:` URI image alongside the tool schema) and a new system prompt telling the model to replicate every visible class/attribute/operation/relationship, creating classes before their members and both endpoint classes before a relationship between them.
- **Explicitly OpenAI-only, on purpose**: `GeminiClient`'s implementation of the same protocol method raises a clear `GeminiRequestError` ("not supported with the Gemini provider yet") instead of attempting real Gemini vision support — a deliberate scope cut given limited remaining time, not an oversight. Anyone with `LLM_PROVIDER=gemini` gets a clean 502 explaining why, via the exception handler that already existed, rather than an `AttributeError`.
- **Basic input hygiene**: the endpoint rejects anything over 10 MB or outside `image/{png,jpeg,webp,gif}` before it ever reaches OpenAI, via a new `InvalidImageError` → 400 `invalid_image`.
- **Process note**: the delegated builder self-reported that it did not follow strict red-green TDD for the backend half (wrote implementation and tests close together rather than watching each test fail first) — flagged explicitly since this project runs in Strict TDD Mode; all tests do pass and were independently re-run and confirmed by this session (not just trusted from the report), but the red-green discipline itself was not verified this time. Worth enforcing more explicitly in the next delegation brief.
- Verified (by the delegated builder, and independently spot-checked here): `apps/ai_assistant` backend tests 89 passed, full backend suite 1455 passed, frontend 429 passed, `tsc --noEmit` clean. NOT yet verified against a real uploaded image — that's the next thing to try before trusting this in front of anyone else; the real risk is prompt/extraction accuracy on a messy or hand-drawn image, not the plumbing.

## 2026-09-22 — Offline write-queue for `screen_designer`'s mutation buttons (DD182, delegated build, no SDD)

- **DD182** A `create`/`update`/`delete` button on `RuntimeScreenPage` that fails because the request never left the device (`SocketException`/`http.ClientException`) now queues locally (`PendingActionStore`, JSON list in `shared_preferences`, mirrors the existing store pattern) instead of just showing an error. `PendingActionSyncService` replays the queue — on `connectivity_plus`'s connectivity-restored event (a retry HINT only, never proof it'll work) or a manual tap on the AppBar's "N pending" indicator — through the same `ScreenActionClient`, independently per entry (no rollback on a partial failure, same convention as the web voice assistant's multi-command handling).
- **Deliberate scope limit, not an oversight**: only a genuinely-unreachable failure gets queued. A timeout or a 4xx/5xx response is surfaced as an error exactly as before and is NEVER auto-retried — the generated backend has no idempotency key, so blindly retrying a request that may have actually reached the server risks creating a duplicate record. `ScreenActionFailure` gained an `isUnreachable` field (named, defaults to `false`) to carry this distinction without breaking any existing call site.
- `list` (read) actions are never queued — a read has no "pending" semantics.
- Verified for real on the user's phone: airplane mode → `create` → queued with a visible "1 pending" indicator → airplane mode off → synced automatically, confirmed by the user ("funciona de 10"). `flutter analyze`: clean. `flutter test`: 105/105 passed (18 new), confirmed independently by re-running both commands, not just trusting the delegated builder's report.

## 2026-09-22 — On-device AI assistant, step 1 of 3: local LLM pipeline proven on a real phone (DD181, delegated build + direct fix, no SDD)

- **DD181** New `mobile/lib/features/local_assistant/`: a settings screen where the user pastes their own Hugging Face access token (stored locally via `shared_preferences`, never sent anywhere but Hugging Face's own download endpoint) and downloads `Gemma3-1B-IT` (584 MB, quantized `.litertlm`) fully on-device via `flutter_gemma` + `flutter_gemma_litertlm` (the LiteRT-LM engine — MediaPipe's Android/iOS LLM Inference API is deprecated in favor of it, confirmed live from Google's own docs). A minimal test chat screen proves the pipeline end to end with no server involved. This is step 1 of 3: proving the raw on-device pipeline works before wiring real function-calling to `screen_designer` (step 2) or building the offline OpenAPI cache (step 3) — mobile has no diagram editor at all, so this assistant is scoped to what mobile actually has, not a port of the web voice assistant's `UmlCommand` schema.
- **Real discrepancy from the docs**: `flutter_gemma` is a federated plugin core with NO inference engine built in — `installModel()`/`getActiveModel()` throw until a second package (`flutter_gemma_litertlm`) registers the actual engine via `FlutterGemma.initialize(inferenceEngines: [LiteRtLmEngine()])`. Found by the delegated builder while implementing, not from the docs.
- **Real bug, found live on the actual device, root-caused via `flutter run`'s attached console — not by settings-hunting**: after installing, the whole app rendered with a persistent reddish tint and the new floating action button was invisible. Two device-settings theories were tried and ruled out first (Android's "Debug GPU overdraw" — confirmed off; MIUI/HyperOS's "Game Turbo" auto-classifying the app as a game — confirmed real via `adb logcat`'s `GameBoosterService: gameStatus: 1` firing the instant the app opened, but disabling Game Turbo entirely did not fix the tint). The real cause, found only once `flutter run` streamed the live Dart console: `EXCEPTION CAUGHT BY WIDGETS LIBRARY — No Overlay widget found`, thrown on every single frame by the new `FloatingActionButton`'s built-in `Tooltip`. `MaterialApp.builder` (used to give the assistant an always-reachable entry point without touching `api_connection`/`screen_designer`) runs OUTSIDE the `Overlay`/`Navigator` that `MaterialApp` creates internally for `home`/routes, so nothing built inside that `builder` has an `Overlay` ancestor unless one is added explicitly. Fixed by wrapping the `builder`'s `Stack` in its own `Overlay`/`OverlayEntry` (`mobile/lib/app.dart`). Lesson: `MaterialApp.builder` content that needs an `Overlay` (tooltips, banners, text-selection toolbars) needs its own, and a repeated per-frame widget-build exception is a real, verifiable explanation for a persistent rendering artifact — worth ruling in/out via `flutter run`'s live console before guessing at device settings.
- **MIUI/HyperOS install friction, for next time**: `flutter install`/`adb install` both failed with `INSTALL_FAILED_USER_RESTRICTED` on this device even with the standard USB-debugging toggles on — no on-device dialog appeared to approve. Worked around by `adb push`ing the APK straight to `/sdcard/Download/` and installing by hand from the Files app (needed `MSYS_NO_PATHCONV=1` in Git Bash for the `/sdcard/...` path; and the flutter-level device id `flutter devices` shows is NOT the same string raw `adb -s` needs for a wireless connection — get the real one from `adb devices -l`).
- **Verified for real, fully offline, on the user's own hardware**: Xiaomi POCO F8 Pro, Snapdragon 8 Elite, 12 GB RAM. Model downloaded once (needs internet + a Hugging Face account with the Gemma license accepted on huggingface.co/litert-community/Gemma3-1B-IT), then the test chat answered correctly with the device's connectivity off — confirmed by the user ("la IA local funciona en modo offline"). `flutter analyze`: clean. `flutter test`: 83/83 passed (6 new).

## 2026-09-22 — Voice-driven diagram assistant, with a swappable LLM provider (DD180, delegated build + direct fixes, no SDD)

- **DD180** New `apps/ai_assistant`: a mic button on the canvas transcribes speech (browser `SpeechRecognition`, `es-AR`), sends the transcript to `POST .../voice-command`, which asks an LLM (function calling over 9 of the 11 `UmlCommand`s — `RemoveClass`/`SetGenerationProfile` excluded, see `tools.py`) to turn it into structured function calls, translates each returned call's class/attribute/relationship *names* into real `ElementId`s (`translator.py`), and submits every one through the EXISTING, already-tested `command_from_payload`/`submit_command` pipeline — the LLM never touches the database directly. Multi-command utterances apply independently in order (a failure at step 2 of 3 does not roll back 1 or block 3). Built by a delegated agent from a fully pre-specified architecture (frontend + backend in one pass, 92 tests, reviewed and spot-verified before merging into this session's own work).
- **Real bug, found live, fixed twice**: the first `GeminiClient.generate_function_calls` call had NO timeout at all (`HttpOptions.timeout` defaults to `None`) — a slow/stuck upstream hung the request (and the "Procesando..." UI) forever. Fixed with an explicit `REQUEST_TIMEOUT_MS` (initially 20s, raised to 45s after real measurement against the live Gemini API showed 19-25s+ round trips for a TRIVIAL prompt from the dev machine's network, high variance run to run — confirmed this was network/latency, not the model "thinking" by testing with `thinking_budget=0`, which made no difference).
- **Provider swap, `llm_provider.py`**: after Gemini failed four different ways in one session from this network/account (404 deprecated model, timeout, `503` high-demand, `504` deadline-exceeded) while a bare connectivity probe to OpenAI answered in 0.43s, added `apps/ai_assistant/openai_client.py` as a second `GeminiClientProtocol` implementation (the protocol is structural/provider-agnostic despite its name) and `LLM_PROVIDER` (`gemini` default, `openai` alternative) to pick between them. `tools.py`'s plain-dict schema is reused as-is for both — OpenAI's function-calling shape just wraps each entry in `{"type": "function", "function": {...}}}`. Cost check for the user's $5 OpenAI credit: `gpt-4o-mini` at $0.15/1M input + $0.60/1M output tokens, ~1500-2000 tokens per voice command → roughly 15,000+ commands, confirmed against OpenAI's own current pricing page.
- **Test-isolation gotcha found while verifying**: two "missing API key" tests only overrode `GEMINI_API_KEY=""`, not `LLM_PROVIDER` — once the developer's real `.env` had `LLM_PROVIDER=openai` (to actually test the OpenAI path), those tests started reading the container's REAL `OPENAI_API_KEY` and stopped raising `MissingApiKeyError`. Fixed by forcing `settings.LLM_PROVIDER = "gemini"` explicitly in both tests — a provider-dispatch test must never depend on ambient environment state beyond what it explicitly overrides.
- Verified for real end-to-end against the live Gemini API and the live OpenAI API (not just mocked): the OpenAI path completed a full round trip in under a second and applied a class correctly; confirmed working "al 100" by the user.

## 2026-09-22 — First real cloud deployment, on AWS EC2 (DD179, small direct change, no SDD)

- **DD179** Live at `https://modelia.duckdns.org`, on AWS EC2 (`c7i-flex.large`, 2 vCPU/4 GiB — the account's plan blocks `t3.medium` and most `t2`/`t3` sizes behind an "upgrade your plan" wall, `c7i-flex.large`/`m7i-flex.large` were the free-tier-eligible substitutes with comparable RAM), Ubuntu 26.04, 30 GiB gp3, an Elastic IP, DuckDNS for the domain, following `deploy/README.md` exactly (Docker install, UFW, `git clone`, `cp env.example .env` + the 4 required secrets, `docker compose -f docker-compose.prod.yml up -d --build`). Caddy issued its Let's Encrypt cert on the first real request, no manual step needed.
- **Real bug found and fixed**: `backend/Dockerfile`'s `prod` stage failed every time with `exec: "./entrypoint.sh": permission denied`. The `base` stage's `RUN chmod +x entrypoint.sh` only applies to that layer; the `prod` stage's later `COPY . /app` re-copies `entrypoint.sh` straight from the build context (not executable on disk in git), silently undoing the earlier chmod. Never caught in `dev`, because the `dev` stage bind-mounts source instead of re-copying it — this only ever manifests in a `prod`-target build. Fix: a second `RUN chmod +x /app/entrypoint.sh` right after the `prod` stage's `COPY . /app`. Verified locally (`docker build --target prod`, confirmed `-rwxr-xr-x` inside the built image) before pushing.
- **Explicit user decision, not a default**: ran `python manage.py seed_demo` on the production database, on the user's explicit request after being warned that `DEMO_PASSWORD = "DemoPass123!"` is hardcoded in public source (`apps/organizations/management/commands/seed_demo.py`) — anyone reading the repo can log in as `owner@demo.com` with full org-owner access on the live deployment. The user asked for it twice, understanding the risk; this is not a recommended production pattern and should not be repeated automatically for a future deploy without asking again.
- Postgres/Redis/the runner's Docker API are deliberately not exposed (`deploy/README.md`'s port table) — a future TablePlus/DB-GUI connection needs an SSH tunnel (`db` has no `ports:` mapping at all in `docker-compose.prod.yml`, not even to the VM's own loopback), not a direct connection to the public IP. Not set up yet.

## 2026-09-22 — Join-table hint is draggable and gets its own connector lines (DD178, small direct change, no SDD, committed as `aa5ad51`)

- **DD178** Follow-up to DD177: the join-table hint node is now grabbable (its `grabbable: false` was removed) and gets two dashed, unlabeled connector edges (`edge.join-table-edge`, styled distinctly from the real association edge, which keeps its own name/multiplicities and stays the only thing double-tap-editable) — one from the relationship's source class to the hint, one from the hint to the target class; a recursive relationship (source === target) draws both to/from the same class, which Cytoscape auto-bends apart as parallel edges. Dragging the hint stashes its new position in a session-only ref (`hintPositionOverridesRef`, keyed by the hint's own id) that `joinTableHintElements`'s new `overridePositionOf` parameter consults ahead of the auto-computed midpoint — so a manual drag survives the next resync (another class moved, a model edit bumped `revision`) instead of snapping back. This position is NOT persisted (no backend field for it) and NOT shared with other collaborators viewing the same document — purely a local, per-tab convenience, same boundary as any other visual-only preview. `joinTableHintElements`'s node+edges are still added via a single `cy.add()` outside `toElements`/fcose, still all tagged `synthetic: true`, and the edge tap handler now also skips synthetic edges (so a double-tap on a connector line can never invoke `onEdgeEdit` with a fake id).

## 2026-09-22 — Diagram canvas previews a many-to-many association's join table (DD177, small direct change, no SDD, uncommitted)

- **DD177** New `GET /api/orgs/{org_slug}/documents/{doc_id}/join-tables` (`apps/relational_mapping/api.py`, new — this app had a mapper but no router yet) runs the real `map_to_relational` and returns every table with `not source_class_ids` (the join-table shape `_build_join_table` always produces): `{relationship_id, table_name, columns}`. `uml_documents` is not allowed to import `apps.relational_mapping` (its own import-boundary test), so this lives in `relational_mapping` itself, which already may depend on `uml_documents` (it needs `services.get_document` to load the model) — the same direction `generation_export` already uses. Reusing the real mapper, rather than reimplementing its naming rules in TypeScript, is what guarantees the drawn table never drifts from what `Generar backend` actually produces (confirmed against a real diagram with a `Class A <-> Class B` and a self-referencing `C <-> C` many-to-many: `class_a_class_b` and `c_c` both rendered with their exact real column names). `DiagramCanvas` renders each hint as a dashed-border box (`classBoxSvgDataUri`, styled `.join-table-hint`) at the midpoint between its relationship's two endpoint CURRENT positions (`cy.getElementById(id).position()`, not the persisted layout — works even before either class was ever dragged) with a small perpendicular offset, added/repositioned via `cy.add`/`cy.$(...).remove()` OUTSIDE `toElements`/fcose (fcose would otherwise treat it as a real node to place). The node carries `data.synthetic = true` and `grabbable: false`; `applyLocks` and the `tap`/`grab`/`drag`/`free` handlers all skip it, since it is not a real, editable class.

## 2026-09-22 — Mobile Field picker offers real attribute names (DD176, small direct change, no SDD, uncommitted)

- **DD176** `EndpointGroup` gained `attributeNames` (default `[]`), resolved in `openapi_parser.dart` from that class's `POST` operation's request body: `requestBody.content['application/json'].schema.$ref` resolved against `components.schemas.<name>.properties`. Resolution is deliberately tolerant — any missing/inline/oddly-shaped schema yields `[]`, never throws, since a class with no attribute list must degrade to typing the name by hand, not break discovery. The designer's "Add field"/"Edit field" flow now shows a class's real attributes as a picker (mirroring the existing class picker) whenever the list is non-empty, falling back to free text otherwise. Verified for real: against the live backend's schema, Class A's picker showed exactly `edad`, `classAId`, `name` — its real DTO fields, not a guess.

## 2026-09-22 — Mobile screen designer mixes several classes on one screen (DD175, small direct change, no SDD, uncommitted)

- **DD175** `ScreenLayout` dropped its single `groupName` (a screen was scoped to one class) for a user-given `name`; `FieldWidgetConfig`/`ButtonWidgetConfig` each carry their own `groupName` instead, so one screen can freely mix widgets from several classes. `ScreenDesignerPage` now takes the WHOLE `groups: List<EndpointGroup>` (not one `group`) and asks a "Class" picker before adding/rebinding a Field or Button, but only when more than one group exists — a single-class screen keeps its original, simpler add flow with no extra prompt. `RuntimeScreenPage` resolves each button's collection path from its OWN `groupName` and gathers request-body values only from same-`groupName` Field widgets, so a create/update on one class can never leak another class's typed values into its body (verified for real: two create buttons on one mixed screen produced two separate POSTs, each with only its own class's field). `ScreenLayoutStore` now indexes multiple named screens (`listNames`/`load`/`save`/`delete`) instead of one layout per class. Entry points: tapping a group in Discovery suggests its name for a new screen but the designer can then add ANY class; a new "open a saved screen" folder icon lists and reopens any previously saved screen by name.

## 2026-09-22 — Mobile RAD-style screen designer (DD174, small direct change, no SDD, uncommitted)

- **DD174** New `mobile/lib/features/screen_designer/`, built live in the app (the user's own reference: Delphi/RAD Studio's form designer), not on the web — the discovery screen's group names are now tap targets. A `ScreenLayout` (a list of `Label`/`Field`/`Button` widgets, each with an id, an x/y and a binding) is the whole contract: no Dart codegen, no rebuild to see a change. Tapping the palette adds an already-configured widget (through a small dialog) rather than a separate blank-placeholder drag; a placed widget still drags freely via `onPanUpdate` to reposition, and a tap reopens its dialog to rebind, retype or remove it. Persistence is `shared_preferences`, keyed by group name only (MVP: two different backends with a same-named group share a design — acceptable for a single-backend-at-a-time workflow). `RuntimeScreenPage` renders a saved layout for real: a Button's action (list/create/update/delete) calls `ScreenActionClient` against the group's actual collection endpoint, derived from its `POST` endpoint's path (this project's Spring generator convention is `/api/<resource>` + `/api/<resource>/{id}`); update/delete additionally prompt for a record id, since there is no data grid yet to pick one from. Field widgets are plain `TextField`s the user types into — no schema-driven field list yet (deferred: the OpenAPI parser doesn't resolve request-body schemas). `deploymentBaseFrom(openApiUri)` (new, in `api_connection/domain/openapi_url.dart`) strips the trailing `/v3/api-docs` to get the deployment's own base URL that `/api/...` paths resolve against — this is NOT the same URL as `openApiUri` itself. Verified for real against a live generated backend (Chromium with `--disable-web-security`, since Flutter Web's fetch is otherwise CORS-blocked by the generated backend, which has no CORS headers by design — native Android has no such restriction): connect, open a group's designer, add/drag/save a field+button, reopen to confirm the save survived, preview, type a value, submit a real POST (received a real 400 from the backend, correctly displayed — the typed attribute name did not match the real schema, expected given no field autocomplete).
