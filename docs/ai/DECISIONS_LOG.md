...
- **DD147 (tech debt)** the declared `crud` does NOT filter `operations[]`, which keeps describing what the generated controller actually serves (all six operations); the generator ignores `crud` today. Revisit when the generator consumes `crud`.
