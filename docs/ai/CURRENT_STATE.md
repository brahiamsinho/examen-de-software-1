...
**Update 2026-09-22 (newest; join-table hint is draggable + gets connector lines, uncommitted):** see DD178, a follow-up to DD177 below. The join-table hint node can now be dragged (its position sticks, session-only, not persisted or shared) and gets two dashed connector lines to its relationship's two endpoint classes, so it reads as attached to the relationship instead of floating unconnected — the real association edge (name + multiplicities, double-tap-editable) is untouched. Frontend-only change (`DiagramCanvas.tsx` + its test file). Frontend 420 passed (incl. 2 new tests), `tsc --noEmit` clean. Not yet verified in a live browser this round — the drag interaction is worth trying by hand.
