...
**Update 2026-09-22 (newest; import a diagram from an image via OpenAI vision, committed):** see DD183. New `POST /{doc_id}/image-command` + "Importar desde imagen" button on the canvas, reusing ~90% of DD180's voice-command pipeline (same tool schema, translator, command-submission loop, response shape) — the only new pieces are `OpenAiClient.generate_function_calls_from_image` (multimodal Chat Completions) and a system prompt for replicating a diagram from a picture. OpenAI-only on purpose; `LLM_PROVIDER=gemini` gets a clean 502, not a crash. Backend `apps/ai_assistant` 89 passed, full backend suite 1455 passed, frontend 429 passed, `tsc` clean (independently re-confirmed, not just trusted from the delegated report). **Not yet tried against a real uploaded image** — do that first before relying on it; extraction accuracy on a messy/hand-drawn diagram is the real open question, not the plumbing.

**Update 2026-09-22 (offline write-queue for screen_designer, committed):** see DD182. `create`/`update`/`delete` buttons on `RuntimeScreenPage` now queue locally (`PendingActionStore`/`PendingActionSyncService`, new in `screen_designer/data/`) when the request genuinely never left the device, and auto-sync when connectivity returns (or on a manual tap of the new "N pending" AppBar indicator) — a timeout or a server error is still surfaced immediately and is never auto-retried, to avoid creating duplicate records against a backend with no idempotency key. `flutter test`: 105/105 passed. Verified for real: airplane mode → create → queued → airplane mode off → synced, confirmed by the user.

**Update 2026-09-22 (on-device AI assistant, step 1 of 3, committed):** see DD181. New `mobile/lib/features/local_assistant/` — download `Gemma3-1B-IT` fully on-device via `flutter_gemma`/`flutter_gemma_litertlm` with the user's own Hugging Face token, then a minimal test chat proves the local LLM pipeline works with no server involved. Found and fixed a real bug along the way: the new floating-action-button entry point lived in `MaterialApp.builder`, outside the app's own `Overlay`, so its `Tooltip` threw on every frame and painted the whole screen with Flutter's error tint — fixed by giving the `builder`'s content its own `Overlay` (`mobile/lib/app.dart`). Verified for real, fully offline, on the user's own Xiaomi POCO F8 Pro (Snapdragon 8 Elite). `flutter test`: 83/83 passed, `flutter analyze` clean. Steps 2 (wire this to `screen_designer` via function calling, switching to the `FunctionGemma` model) and 3 (offline OpenAPI cache) not started.

**Update 2026-09-22 (voice-driven diagram assistant, working end-to-end, committed):** see DD180. New `apps/ai_assistant` + `VoiceCommandButton` on the canvas: speak an instruction ("creá una clase Persona con un atributo edad"), it's transcribed in-browser, sent to an LLM (function calling over 9 UML commands), translated to real command payloads, and applied through the existing validated pipeline. Two providers wired (`LLM_PROVIDER=gemini|openai`) after Gemini proved unreliable from the dev machine's network (404 deprecated model → 503 high demand → 504 deadline exceeded, across two different models) while OpenAI worked cleanly. Backend 1431 passed, frontend 425 passed, `tsc --noEmit` clean. **Confirmed working for real by the user against the live OpenAI API** ("funciona al 100").

**Update 2026-09-22 (first real cloud deployment, committed):** see DD179. Modelia is live at `https://modelia.duckdns.org` — AWS EC2 (`c7i-flex.large`, Ubuntu 26.04, 30 GiB, Elastic IP), DuckDNS domain, `docker-compose.prod.yml` (Caddy auto-HTTPS + backend + frontend + runner + db + redis + socket-proxy). Found and fixed a real bug on the way: `backend/Dockerfile`'s `prod` stage's `COPY . /app` was silently undoing the base stage's `chmod +x entrypoint.sh` (never caught in `dev`, which bind-mounts instead of re-copying) — fixed with a second `chmod +x` after that `COPY`, verified locally before pushing. `python manage.py seed_demo` was run on production at the user's explicit request, after being warned its password is public in source — not a pattern to repeat automatically. Postgres/Redis are not reachable at all from outside the VM's Docker network (by design) — a DB-GUI tool needs an SSH tunnel, not set up yet.

**Update 2026-09-22 (join-table hint is draggable + gets connector lines, committed as `aa5ad51`):** see DD178, a follow-up to DD177 below. The join-table hint node can now be dragged (its position sticks, session-only, not persisted or shared) and gets two dashed connector lines to its relationship's two endpoint classes, so it reads as attached to the relationship instead of floating unconnected — the real association edge (name + multiplicities, double-tap-editable) is untouched. Frontend-only change (`DiagramCanvas.tsx` + its test file). Frontend 420 passed (incl. 2 new tests), `tsc --noEmit` clean. Pushed without a live-browser check (user chose to skip it and ship) — worth trying by hand on `https://modelia.duckdns.org`.

**Update 2026-09-22 (diagram canvas previews a many-to-many's join table, committed as `883301a`):** see DD177. New backend `GET .../documents/{doc_id}/join-tables` (`apps/relational_mapping/api.py`) runs the real `map_to_relational` and returns every join table (both-ends-many association) with its relationship id, name and columns. `DiagramCanvas` draws each as a dashed, non-editable box at the midpoint between its two classes (frontend `lib/join_tables.ts`, `documents/[docId]/page.tsx` fetches on every `document.revision` change). Backend 1366 passed, frontend 419 passed, `tsc --noEmit` clean. Verified for real in a browser: a `Class A <-> Class B` and a self-referencing `C <-> C` many-to-many both rendered their real join table (`class_a_class_b`, `c_c`) with the exact columns the generator produces.

**Update 2026-09-22 (mobile Field picker offers real attribute names, uncommitted):** see DD176. `EndpointGroup.attributeNames` (resolved from the class's POST request body schema) lets the designer's "Add field"/"Edit field" show a picker of the class's real attributes instead of the user typing a name by hand — degrades to free text for a class with no resolvable schema. `flutter test`: 77/77 passed. `flutter analyze`: clean. Verified for real against a live backend: Class A's picker showed its actual fields `edad`, `classAId`, `name`. APK pushed to the phone, not yet confirmed run there.

**Update 2026-09-22 (mobile screen designer mixes several classes, committed as `ea31a64`):** the screen designer (DD174, committed as `862184a`) no longer scopes a screen to one class. See DD175. A screen has its own name (`ScreenLayout.name`); each Field/Button carries its OWN `groupName`, so one screen freely mixes classes — e.g. a "create" button for Class A next to one for Class B, each sending only its own fields. The designer's palette asks a "Class" picker before adding/rebinding a Field or Button whenever more than one class is available (skipped when there's only one, keeping that case as simple as before). `ScreenLayoutStore` now holds multiple named screens; Discovery's new folder icon lists and reopens any of them. `flutter test`: 74/74 passed. `flutter analyze`: clean. Verified for real (CORS-disabled Chromium against a live generated backend): built a screen mixing Class A and Class B, tapped each class's own "create" button, and confirmed via captured network traffic that each POST carried only its own class's field (no cross-class leakage). The updated APK was pushed to the user's phone (`adb push` + manual tap-install, same as before) but not yet confirmed run there.

**Update 2026-09-22 (mobile screen designer, single class per screen; committed):** the Flutter app can now design and run a small screen against a generated backend's endpoints, in-app — the user's own reference is Delphi/RAD Studio's form designer. From the discovery screen, tapping a group name (e.g. `class-a-controller`) opens `ScreenDesignerPage`: a palette (Label/Field/Button) plus a canvas where placed widgets drag to reposition and tap to rebind/retype/remove. "Save" persists the design locally (`shared_preferences`, keyed by group name); "Preview" (or reopening later) runs `RuntimeScreenPage`, which renders the saved layout for real and wires each Button to a genuine `list`/`create`/`update`/`delete` call against the connected backend (`update`/`delete` prompt for a record id — no data grid yet). See `DECISIONS_LOG.md` DD174. `flutter test`: 68/68 passed. `flutter analyze`: clean. Verified against a live generated backend in a real (CORS-disabled) Chromium session: full connect -> design -> drag -> save -> reload -> preview -> real network POST round trip. Not yet: field name autocomplete from the OpenAPI schema (schemas aren't parsed yet), a data grid for update/delete, and reinstalling the updated APK on the user's phone (the previous stage's build is what is currently on it).

**Update 2026-09-21 (edit a relationship on the canvas; small direct change, no SDD, uncommitted):** double-clicking a relationship (its line or any of its three labels) on the diagram canvas opens an "Editar relación" dialog to change its name and both multiplicities; generalizations show only the name. Backend: new `UpdateRelationship` command (`relationship_id`, optional `name` where null/blank clears it and an omitted key leaves it, optional `source_multiplicity`/`target_multiplicity` as UML strings), handler in `uml_commands/handlers/relationships.py`, schema `UpdateRelationshipIn`, and a write-time gate in `uml_documents/services.py` (unknown id or multiplicities on a generalization -> 422 `invalid_command_payload`). Frontend: `EditRelationshipDialog` (new), `parseMultiplicityInput` in `lib/uml_documents.ts`, `onEdgeEdit` prop on `DiagramCanvas`, dialog owned by the document page and shown only to OWNER/EDITOR. Verified in a real browser (edit, reload, persisted). Backend 328 and affected vitest files pass, `tsc --noEmit` clean.

**Update 2026-09-21 (newest; Flutter stage 1 `mobile-openapi-connect` DONE, uncommitted):** the Flutter app now connects to a generated backend by URL. `mobile/lib/features/api_connection/` implements all five layers against the pre-existing RED tests (domain `openapi_url.dart`/`connection_failure.dart`/`api_discovery.dart`, data `openapi_parser.dart`/`openapi_client.dart`, presentation `connect_controller.dart`/`connect_page.dart`/`discovery_page.dart`), plus `lib/app.dart`, `lib/main.dart`, and `GENERATED_API_URL` in `core/config/app_config.dart`. Android: `INTERNET` added to the main manifest (a release build had none before), `usesCleartextTraffic="true"` added to the debug manifest only. `flutter test`: 47/47 passed. `flutter analyze`: 0 issues (two `prefer_initializing_formals` infos silenced with `// ignore:` — the public `client:` parameter name must stay, tests depend on it). `flutter build web` compiles clean. No real-deployment manual smoke was run (task 8.2 is conditional on one already running; none was). Flutter SDK is installed at `C:\Users\brahi\flutter` (`flutter --version` works, no PATH entry); no Android SDK/emulator available in this environment, only Windows desktop and Edge web targets.

**Update 2026-09-21 (root environment consolidation; uncommitted):** the ignored root `.env` is now the sole real configuration file and root `env.example` (no leading dot) is its committed template. Dev Compose injects it into backend/frontend; production Compose interpolates it while keeping service environments explicit, so secrets do not reach Caddy/frontend. Obsolete templates were deleted (`backend/env.example`, `frontend/env.local.example`, `runner/env.example`, `deploy/env.prod.example`). Config checks passed: dev with `docker compose --env-file env.example -f docker-compose.yml config -q`; production required temporary shell-only `RUNNER_TOKEN=config-check` because the template intentionally leaves it blank, then passed with the analogous production command. No services changed and no commit was made on `chore/unify-root-env`.

**Update 2026-09-21 (diagram delete + XML into a blank diagram; small direct change, no SDD, uncommitted):** `DELETE /api/orgs/{org_slug}/documents/{doc_id}` (apps/uml_documents, OWNER/EDITOR, VIEWER 403, 404 like the other document endpoints, 204) stops the document's active deployments best-effort through `backend_deployments.service.stop_active_for_document` (called from the view, so `uml_documents.services` keeps its import boundary; a runner outage never blocks the delete) and hard-deletes the row; `Deployment.document` is `CASCADE`, nothing else references a document. The websocket layout-release path ignores a document deleted mid-drag. `POST .../documents/{doc_id}/import-xmi` (apps/xmi_interop, OWNER/EDITOR, same 5 MB/422 handling as the dashboard import, shared `_parse_upload`) loads the XMI INTO a blank document via `services.replace_blank_document_content` (row lock, one revision bump, name kept, broadcast on commit) and answers 409 `document_not_empty` when it already has classes or relationships. Frontend: `DeleteDiagramDialog` (confirmation, friendly 403/404), trash button per card in `DocumentGrid`, `Eliminar diagrama` in the editor header (hidden for VIEWER via the org role atom; after delete `router.replace("/dashboard")`), `useDocumentActions().deleteDiagram` invalidates sidebar and grid, and an "Este diagrama esta vacio" call to action with `Importar XML` over the empty canvas (`useDocument().applyDocument` adopts the response; warnings go to the existing alert). Backend 176 passed in uml_documents+xmi_interop+backend_deployments; frontend affected files and `tsc --noEmit` clean; driven in Chromium (blank -> import renders 2 classes/1 relationship; second import 409; delete from card and from editor).
...
**Update 2026-09-21 (newest; small direct change, no SDD, uncommitted):** slice 1 of "run the generated backend": a document can now be deployed and left RUNNING behind a URL. New Django app `apps.backend_deployments` (model `Deployment`: uuid, document/organization FKs, status queued|building|starting|running|failed|stopped, error, log_tail, public_path, created_by; migration `0001`) with `POST /api/orgs/{org_slug}/documents/{doc_id}/deployments` (202; replaces the document's active deployment; per-org quota `DEPLOYMENT_MAX_ACTIVE_PER_ORG`, default 2 -> 409 `quota_exceeded`; 422 `nothing_to_generate|generation_failed`; 502 `runner_unavailable|runner_rejected`; OWNER/EDITOR), `GET .../deployments/latest` (any member; pulls the runner state into the row, so it is the poll target) and `DELETE .../deployments/{id}`. Django never touches Docker: `runner_client.py` (stdlib urllib, fake injected in tests) calls the new root service `runner/` (FastAPI + docker SDK + httpx; compose service `runner`, the ONLY holder of `/var/run/docker.sock`, port bound to `RUNNER_BIND` default `127.0.0.1:8090`, bearer `RUNNER_TOKEN`, refuses to start when empty). The runner validates and unpacks the zip into a per-deployment volume, builds with the pinned Gradle image (named Gradle cache volume), then starts Postgres + a JRE 21 container on a per-deployment `internal` network (fixed image allowlist, cap_drop ALL, no-new-privileges, mem/cpu/pids limits, read-only app rootfs, labels `modelia.deployment`/`modelia.org`), sweeps orphans at startup, reaps after `RUNNER_MAX_LIFETIME_SECONDS` (6h) and reverse-proxies `/gen/{id}/...` (prefix stripped, `X-Forwarded-Prefix`, `SERVER_FORWARD_HEADERS_STRATEGY=framework`). `PUBLIC_BASE_URL` (default `http://localhost:8090`) is read by both Django and the runner; env knobs are listed in `runner/env.example` and `backend/env.example`; `RUNNER_TOKEN` lives in the root `.env`. Verified for real on `ea-basico.xml`: build + boot about 90 s, `/gen/<id>/api/class-as` list 200, POST 201, GET by id 200, a second deploy replaced the first, DELETE left only the Gradle cache volume. Tests: backend 1325 -> 1337 passed; runner 15 passed (`docker build --target test -t modelia-runner-test runner && docker run --rm modelia-runner-test`). Known gaps: the generated project ships only springdoc API docs (`/gen/<id>/v3/api-docs`), no Swagger UI; `RUNNER_BUILD_IMAGE` default must stay equal to `gradle_runner_image()`; no frontend yet.

**Update 2026-09-20 (previous; small direct change, no SDD, uncommitted):** class/attribute/role names typed with spaces, accents, dashes or symbols (e.g. the EA fixture `Class A` / `Class B`) now generate a valid, compilable Spring project instead of failing with `422 Table 'class b' yields an invalid resource path segment`. The fix lives at the single choke point where model names become identifiers, `snake_case` in `apps/relational_mapping/mapping/naming.py`: NFKD accent stripping, every non-alphanumeric run becomes one `_`, edge underscores are trimmed, a leading digit gets an `n_` prefix, and a name with no usable character raises the new `InvalidElementNameError` (an `UnmappableModelError`, so the API still answers 422 `generation_failed`). Names already made of `[A-Za-z0-9_]` convert byte-identically (oracle, sample and drift-guard tests untouched); two names that normalize to the same identifier are disambiguated by the existing `unique_name` (`class_a`, `class_a_2`). Java (`ClassA`), REST (`/api/class-bs`) and Domain Manifest names derive from the table/column names, so they stay consistent; `screaming_snake_case` also strips accents for enum labels. UML model names are never modified. Proof: imported `ea-basico.xml` through the running stack, downloaded `/generate`, unzipped it and `gradle build` (gradle:9.7.1-jdk21) finished BUILD SUCCESSFUL (jar produced). Backend 1308 -> 1325 passed; two tests that encoded the old bug (`Order Item` -> 422) now use a name with no usable characters.

**Update 2026-09-20 (previous; small direct change, no SDD, uncommitted):** Enterprise Architect XMI import/export (mandatory scope, spec §16). New app `apps.xmi_interop` (`importer.py` bytes -> `ImportResult(model, layout, warnings, name)` via `defusedxml`, 5 MB cap, XXE/entity expansion rejected; `exporter.py` `ProjectDocument` -> XMI 1.1/UML 1.3 windows-1252 mirroring the real EA sample, deterministic, with `UML:Diagram` geometry from the layout; `errors.py`; `api.py`). `POST /api/orgs/{org_slug}/documents/import-xmi` (multipart, OWNER/EDITOR, creates a NEW document, returns `DocumentOut` + `warnings`) and `GET .../{doc_id}/export-xmi` (any member); errors are 422 `{detail, code: invalid_xmi|unsupported_xmi}`; the router is registered BEFORE `documents_router` because its GET `/{doc_id}` would otherwise answer 405 to the literal `/import-xmi`. `uml_documents.services.create_document_from_model` added (boundary test unchanged). Import policy: unsupported elements, unknown types (fallback String), class-typed attributes, dangling associations and canonical validation diagnostics become Spanish warnings, never failures; EA class boxes map to centre positions. Frontend: `ImportXmiControl` (dashboard), `ImportWarningsAlert` (document page, warnings handed over via sessionStorage), `Exportar XML` reusing `DownloadBackendButton` with `label` props, `lib/xmi_interop.ts`, shared `lib/save_blob.ts`. Backend 1285 -> 1308 passed; frontend 363 -> 369 passed. Verified against the real EA sample end to end (import 201, export, re-import, generate). **Verified only against ONE real EA file (XMI 1.1, classes/attributes/one association):** XMI 2.1 import is basic and tested with a hand-written fixture only; export of enumerations, generalizations, aggregation/composition and diagram connectors is untested inside real EA. Class names with spaces still fail generation (422 `generation_failed`).
