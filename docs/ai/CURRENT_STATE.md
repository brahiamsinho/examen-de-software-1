...
**Update 2026-09-21 (IN PROGRESS, newest; Flutter stage 1 `mobile-openapi-connect`, SDD hybrid, uncommitted):** planning is complete and the RED tests are written, but NO production code exists yet. Change artifacts: `openspec/changes/mobile-openapi-connect/{proposal.md,design.md,tasks.md,specs/mobile-api-connection/spec.md}` (16 tasks, none checked). Done in `mobile/`: `http: ^1.2.0` added to `pubspec.yaml` (`pub get` not run yet), the template `test/widget_test.dart` deleted, and five RED test files written under `mobile/test/features/api_connection/` (`domain/openapi_url_test.dart`, `data/openapi_parser_test.dart`, `data/openapi_client_test.dart`, `presentation/connect_controller_test.dart`, `presentation/connect_page_test.dart`). `lib/main.dart` is still the counter template and `lib/features/` does not exist, so `flutter test` fails to compile: that is the expected RED, do not "fix" it by weakening the tests. Delivery: `size:exception` (~900 lines) with two work-unit commits (A: domain + parser + client; B: controller + UI + app wiring + Android manifests + docs); no commit has been authorized. Environment blocker: there is NO Flutter SDK on this machine and the Docker image `ghcr.io/cirruslabs/flutter:stable` cannot be pulled (`unexpected EOF` at exactly 10 MB, repeated). Workaround in progress: Flutter 3.47.5 stable (Dart 3.13.4) zip (1,933,437,428 bytes) downloaded with resumable `curl -C -` to `C:\src\flutter_windows.zip`, to be unzipped to `C:\src\flutter` and run as `C:\src\flutter\bin\flutter.bat` (not on PATH; unit/widget tests need no Android SDK). Decisions of the design: keep the URL path prefix (`/gen/<id>`), `http` package only, `ChangeNotifier` + sealed states, no `dart:io` in lib, cleartext HTTP only in the Android debug manifest.
