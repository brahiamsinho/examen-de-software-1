...
- **Update 2026-09-21 (IN PROGRESS, read first; Flutter stage 1 `mobile-openapi-connect`):** the user asked for a manual connection from the Flutter app to a generated backend (URL -> `/v3/api-docs` -> validate OpenAPI 3 -> discovery screen). SDD is in `hybrid` store, `automatic` execution, delivery `size:exception` with two work-unit commits, and the user forbids sub-agents (run phases inline). State: proposal/spec/design/tasks are done (`openspec/changes/mobile-openapi-connect/`, native `gentle-ai sdd-status` says `nextRecommended: apply`), five RED test files exist, no production code yet. Resume at task 1.1 of `tasks.md`: (1) make Flutter runnable: check `C:\src\flutter\bin\flutter.bat`; if missing, finish or resume `curl -fL -C - -o C:\src\flutter_windows.zip https://storage.googleapis.com/flutter_infra_release/releases/stable/windows/flutter_windows_3.47.5-stable.zip` (1,933,437,428 bytes) and unzip into `C:\src` (creates `C:\src\flutter`); Docker image pull is a dead end here; (2) `cd mobile && C:\src\flutter\bin\flutter.bat pub get`, then `flutter test` and CONFIRM RED; (3) implement slice A (`domain/connection_failure.dart`, `domain/openapi_url.dart`, `domain/api_discovery.dart`, `data/openapi_parser.dart`, `data/openapi_client.dart`) until its three test files pass, (4) slice B (`presentation/connect_controller.dart` with sealed `ConnectIdle/ConnectLoading/ConnectSuccess/ConnectFailed`, `connect_page.dart`, `discovery_page.dart`, `lib/app.dart`, `main.dart`, `core/config/app_config.dart` `GENERATED_API_URL`), (5) Android `INTERNET` in main manifest and `usesCleartextTraffic` only in `src/debug`, (6) `flutter analyze` + `flutter test` green, (7) update these docs, mark tasks, verify and archive. The test files define the API contract (class names, messages such as `Enter a valid http:// or https:// URL.`, `Could not reach the server. Check the URL and your network.`, button `Connect`, title `Connect to generated API`, field label `Public URL or OpenAPI URL`, discovery groups rendered expanded). Rules: never `docker compose down`, never commit or push without explicit user authorization, no AI co-author lines, artifacts in English, chat in Rioplatense Spanish. Session note: `docs/ai/sessions/2026-09-21-claude-mobile-openapi-connect.md`.
