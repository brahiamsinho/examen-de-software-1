...
- **Update 2026-09-22 (import a diagram from an image via OpenAI vision, committed):** see `CURRENT_STATE.md`/DD183. Backend: `apps/ai_assistant/{errors,gemini_client,openai_client,service,api}.py` all touched (see DD183 for the split — `_apply_translated_commands` is now shared between the voice and image paths). Frontend: `lib/image_command.ts` (new), `components/workspace/ImageImportButton.tsx` (new), `app/(app)/documents/[docId]/page.tsx` (wired next to the voice-command card). **Do this first if picking this up: test with a real uploaded image** — nobody has tried it against actual OpenAI yet, only against mocked/faked clients in tests. If accuracy is poor on messy images, the system prompt in `service.py`'s `_IMAGE_SYSTEM_INSTRUCTION` is the first thing to iterate on. Also note: the delegated builder that wrote this did not follow strict red-green TDD for the backend half (see DD183) — tests are real and passing, just not verified red-first this time.

- **Update 2026-09-22 (offline write-queue for screen_designer, committed):** see `CURRENT_STATE.md`/DD182. Files: `mobile/lib/features/screen_designer/data/{pending_action_store,pending_action_sync_service}.dart` (new), `screen_action_client.dart` (`ScreenActionFailure.isUnreachable`, `SocketException`/`ClientException` classified explicitly), `presentation/runtime_screen_page.dart` (queues on unreachable create/update/delete, AppBar pending-count indicator, connectivity listener started in `initState`/stopped in `dispose`), `pubspec.yaml` (+`connectivity_plus: ^7.3.1`). `connectivity_plus`'s events are a retry HINT only (interface-level, not real reachability) — the actual HTTP call staying the ground truth is the whole point, don't change that if extending this. Scope reminder for next time: this is separate from `api_connection`'s (still nonexistent) read-side offline cache — don't conflate the two when picking up either one.

- **Update 2026-09-22 (on-device AI assistant, step 1 of 3, committed):** see `CURRENT_STATE.md`/DD181. Files: `mobile/lib/features/local_assistant/{data/{local_assistant_token_store,local_model_service}.dart,presentation/{local_assistant_settings_page,local_assistant_chat_page}.dart}`, `mobile/lib/app.dart` (FAB entry point via `MaterialApp.builder`, now wrapped in its own `Overlay` — see DD181 for why that's required), `mobile/pubspec.yaml` (+`flutter_gemma`, +`flutter_gemma_litertlm`). Model used: `Gemma3-1B-IT_multi-prefill-seq_q4_ekv4096.litertlm` (584 MB) — this is the general-purpose instruct model for proving the pipeline, NOT `FunctionGemma`, which is what step 2 (function-calling wired to `screen_designer`) should actually use. `adb`'s real serial for this session's wireless-connected phone differed from what `flutter devices` shows — get it from `adb devices -l` if `adb -s <flutter's id>` fails with "device not found". `flutter install`/`adb install` were blocked by MIUI/HyperOS (`INSTALL_FAILED_USER_RESTRICTED`, no on-device prompt appeared); `adb push` to `/sdcard/Download/` + manual tap-install worked instead (remember `MSYS_NO_PATHCONV=1` in Git Bash for any `/sdcard/...` path, same gotcha as Docker paths).

- **Update 2026-09-22 (voice-driven diagram assistant, committed as `b57c457`):** see `CURRENT_STATE.md`/DD180. New `backend/apps/ai_assistant/` (api.py, service.py, translator.py, tools.py, gemini_client.py, openai_client.py, llm_provider.py) + `frontend/src/components/workspace/VoiceCommandButton.tsx` + `frontend/src/lib/voice_command.ts`. Env vars new in `env.example`/`docker-compose.prod.yml`/`config/settings.py`: `LLM_PROVIDER` (`gemini` default), `GEMINI_API_KEY`/`GEMINI_MODEL` (`gemini-3.6-flash` default — but check what's actually not-yet-deprecated/not-overloaded before relying on that default; `gemini-2.5-flash` and `gemini-3.1-flash-lite` are known-good fallbacks as of this session), `OPENAI_API_KEY`/`OPENAI_MODEL` (`gpt-4o-mini` default). Not yet deployed to the VM — the AWS EC2 `.env` there needs these same additions before the feature works there too. Gotcha for next time: any new "missing API key" test must explicitly force `settings.LLM_PROVIDER` — it silently reads whatever the real `.env` has otherwise, which broke 2 tests once the dev `.env` got real keys in it (both fixed, see DD180).

- **Update 2026-09-22 (first real cloud deployment, committed as `2abf725`):** see `CURRENT_STATE.md`/DD179. Live at `https://modelia.duckdns.org` on AWS EC2. Fixed a real production-only bug in `backend/Dockerfile` (entrypoint.sh losing its executable bit — see DD179 for the exact mechanism, it'll bite again if anyone adds another `COPY` after a `chmod +x` in a later stage). `.env` on the VM (`~/modelia/.env`, not in git) only needs `APP_DOMAIN` + `SECRET_KEY`/`POSTGRES_PASSWORD`/`RUNNER_TOKEN` — `docker-compose.prod.yml` hardcodes everything else (DEBUG, ALLOWED_HOSTS, CORS/CSRF, cookies) from those. Instance type note for next time: this AWS account's plan blocks `t3.medium` and similarly-sized `t2`/`t3` instances behind an "upgrade your plan" wall — `c7i-flex.large` (4 GiB, current-gen) was the free-tier-eligible substitute actually used. `seed_demo` was run against production at the user's explicit, informed request (public demo password); don't repeat that on a future deploy without asking again. TablePlus/DB-GUI access is not set up — needs an SSH tunnel, `db` publishes no port at all right now.

- **Update 2026-09-22 (join-table hint is draggable + gets connector lines, committed as `aa5ad51`):** see `CURRENT_STATE.md`/DD178, follow-up to DD177. Only `components/workspace/DiagramCanvas.tsx` and its test file changed — no backend touch this time. Not verified in a live browser this round (unit-test coverage only: 42 tests incl. two new ones for the connector edges' data shape and the manual-drag override); the drag interaction itself is worth trying by hand before relying on it.

- **Update 2026-09-22 (diagram canvas previews a many-to-many's join table, committed as `883301a`):** see `CURRENT_STATE.md`/DD177. Backend: `apps/relational_mapping/api.py` (new), wired in `config/api.py`, tests in `apps/relational_mapping/tests/{test_api.py,conftest.py}` (new — every app that needs `auth_client` defines its own `tests/conftest.py`, there is no shared root one). Frontend: `lib/join_tables.ts` (new), `components/workspace/DiagramCanvas.tsx` (`joinTableHintElements`, `syncJoinTableHints`, a `node.join-table-hint` style, `synthetic`-tagged nodes skipped by every tap/grab/drag/free handler and by `applyLocks`), `documents/[docId]/page.tsx` (fetch + pass-through). The `DiagramCanvas` "Cytoscape lifecycle (mocked)" tests hand-roll a fake `cy`/node object; extending real behavior there means extending the mock too (`cy.$`, `cy.add`, `node.data`) or those tests break on an unrelated `TypeError`, not a real assertion failure.

- **Update 2026-09-22 (mobile Field picker offers real attribute names, uncommitted):** see `CURRENT_STATE.md`/DD176. Files: `api_connection/domain/api_discovery.dart` (`EndpointGroup.attributeNames`), `api_connection/data/openapi_parser.dart` (`_requestBodyFields`, resolves `$ref` against `components.schemas`), `screen_designer/presentation/designer_page.dart` (`_pickFieldName`/`_promptAttribute`). Ask the user for authorization before committing; the phone APK is one version behind, not yet confirmed run.

- **Update 2026-09-22 (mobile screen designer mixes several classes, committed as `ea31a64`):** see `CURRENT_STATE.md` and DD175. Files touched again: `screen_widget_config.dart`/`screen_layout.dart` (new `groupName` per widget, `ScreenLayout.name`), `data/screen_layout_store.dart` (rewritten: `listNames`/`load`/`save`/`delete`, multiple named screens), `presentation/{designer_page,runtime_screen_page}.dart` (take `groups: List<EndpointGroup>`, class-picker dialog, per-widget path/body resolution), `presentation/discovery_page.dart` (name-prompt + "open a saved screen" folder icon), new shared `presentation/autofocus_text_field.dart` (extracted, reused by all three text-prompt dialogs). All matching tests rewritten. Ask the user for authorization before committing; the APK on the phone is one version behind again (pushed, not yet confirmed run).

- **Update 2026-09-22 (mobile screen designer, single class per screen; committed as `862184a`):** see `CURRENT_STATE.md` and DD174. Files: `mobile/lib/features/screen_designer/{domain,data,presentation}/*.dart` (new), `mobile/lib/features/api_connection/domain/openapi_url.dart` (new `deploymentBaseFrom`), `.../presentation/{connect_page,discovery_page}.dart` (wired navigation), `mobile/pubspec.yaml` (+`shared_preferences`). Tests: `mobile/test/features/screen_designer/**` (new) + `.../api_connection/presentation/discovery_page_test.dart` (new). A plain `tester.drag()`/single `moveBy` undershoots or misses this canvas's `onPanUpdate` (loses the arena to the same detector's `onTap`); use several small `gesture.moveBy` steps via `tester.startGesture`, and assert on the persisted x/y rather than exact screen pixels. Flutter Web's `fetch` is CORS-blocked by the generated backend (no CORS headers by design) — to browser-test the real network path, launch Chromium with `launchPersistentContext(..., { args: ["--disable-web-security"] })`; this does not affect native Android/iOS, which have no CORS restriction. Ask the user for authorization before committing.

- **Update 2026-09-21 (edit a relationship on the canvas; small direct change, no SDD, uncommitted):** see `CURRENT_STATE.md`. Files: backend `uml_commands/{commands,dispatcher}.py`, `handlers/relationships.py`, `uml_documents/{schemas,services}.py` (+ tests); frontend `components/workspace/{EditRelationshipDialog,DiagramCanvas}.tsx`, `lib/uml_documents.ts`, `app/(app)/documents/[docId]/page.tsx` (+ tests). Cytoscape has no double-tap event, so `DiagramCanvas` detects two taps on the same edge within 400 ms and edges use `text-events: yes` so label taps count. Remember `docker compose restart frontend` after frontend edits.

- **Update 2026-09-21 (Flutter stage 1 `mobile-openapi-connect` DONE, uncommitted):** see `CURRENT_STATE.md`. Flutter SDK: `C:\Users\brahi\flutter\bin\flutter.bat` (or `/c/Users/brahi/flutter/bin/flutter` in Git Bash), not on PATH. Files: `mobile/lib/features/api_connection/{domain,data,presentation}/*.dart`, `mobile/lib/{app,main}.dart`, `mobile/lib/core/config/app_config.dart`, `mobile/android/app/src/{main,debug}/AndroidManifest.xml`. All 47 pre-existing RED tests now pass; `flutter analyze` clean; `flutter build web` compiles. Not yet done: a manual smoke against a real `/gen/<id>` deployment (task 8.2, optional, none was running), and the `openspec/changes/mobile-openapi-connect/` folder was left as-is (not archived) per this project's light-SDD-ceremony preference — the living docs here are the source of truth, not the openspec folder. Next stage (not started): Modelia login + deployment selector to replace the manual URL field, and a `mobile-ui.json`-driven dynamic CRUD screen. Ask the user for authorization before any commit.

- **Update 2026-09-21 (root environment consolidation; uncommitted):** root `.env` (ignored) is the only real config and root `env.example` is the committed template; removed `backend/env.example`, `frontend/env.local.example`, `runner/env.example`, and `deploy/env.prod.example`. Dev Compose injects the root file only into backend/frontend; production interpolates it but keeps service environments explicit so Caddy/frontend receive no broad secret injection. VM: copy `env.example` to root `.env`, fill secrets/domain, then run `docker compose -f docker-compose.prod.yml up -d --build` (never `docker compose down`). Both Compose config checks passed; production needed temporary shell-only `RUNNER_TOKEN=config-check` because the template token is intentionally blank. No services changed; no commit on `chore/unify-root-env`.
