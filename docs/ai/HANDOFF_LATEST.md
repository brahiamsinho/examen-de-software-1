...
- **Update 2026-09-22 (join-table hint is draggable + gets connector lines, uncommitted):** see `CURRENT_STATE.md`/DD178, follow-up to DD177. Only `components/workspace/DiagramCanvas.tsx` and its test file changed — no backend touch this time. Not verified in a live browser this round (unit-test coverage only: 42 tests incl. two new ones for the connector edges' data shape and the manual-drag override); the drag interaction itself is worth trying by hand before relying on it.
