...
- **Update 2026-09-22 (voice-driven diagram assistant, uncommitted):** see `CURRENT_STATE.md`/DD180. New `backend/apps/ai_assistant/` (api.py, service.py, translator.py, tools.py, gemini_client.py, openai_client.py, llm_provider.py) + `frontend/src/components/workspace/VoiceCommandButton.tsx` + `frontend/src/lib/voice_command.ts`. Env vars new in `env.example`/`docker-compose.prod.yml`/`config/settings.py`: `LLM_PROVIDER` (`gemini` default), `GEMINI_API_KEY`/`GEMINI_MODEL` (`gemini-3.6-flash` default — but check what's actually not-yet-deprecated/not-overloaded before relying on that default; `gemini-2.5-flash` and `gemini-3.1-flash-lite` are known-good fallbacks as of this session), `OPENAI_API_KEY`/`OPENAI_MODEL` (`gpt-4o-mini` default). Not yet deployed to the VM — the AWS EC2 `.env` there needs these same additions before the feature works there too. Gotcha for next time: any new "missing API key" test must explicitly force `settings.LLM_PROVIDER` — it silently reads whatever the real `.env` has otherwise, which broke 2 tests once the dev `.env` got real keys in it (both fixed, see DD180).
