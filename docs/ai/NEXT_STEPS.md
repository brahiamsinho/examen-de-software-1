...
**Update 2026-09-22 (import a diagram from an image via OpenAI vision):** built and unit-tested (see `CURRENT_STATE.md`, DD183), committed — but **not yet tried against a real uploaded image**, that's the immediate next step before trusting it. Next candidates after that: (1) if accuracy is weak on hand-drawn/messy images, iterate on `_IMAGE_SYSTEM_INSTRUCTION` in `service.py`; (2) surface which provider is active before someone hits the Gemini 502 confused (same still-open item as DD180's NEXT_STEPS note); (3) consider a size/quality hint to the user before upload (e.g. "clearer images work better") once real accuracy is known.

**Update 2026-09-22 (offline write-queue for screen_designer):** done and confirmed working for real on the user's phone (airplane mode round trip), see `CURRENT_STATE.md`/DD182, committed. Next candidates: (1) a visible list of what's actually queued (today just a count — tapping an entry to see/cancel it would help); (2) the still-separate, still-not-started read-side offline cache for `api_connection` (last-fetched OpenAPI schema + URL, so the app can at least open to a "last known" state without connectivity) — don't conflate with this write-queue when picking it up; (3) step 2 of the on-device AI assistant below.

**Update 2026-09-22 (on-device AI assistant, step 1 of 3):** done and confirmed working fully offline by the user on real hardware (see `CURRENT_STATE.md`, DD181), committed. Next: (1) step 2 — wire this to `screen_designer` with real function-calling, switching the downloaded model to `FunctionGemma 270M` (purpose-built for on-device function calling, per `fluttergemma.dev`) instead of the general-purpose `Gemma3-1B-IT` used for step 1's proof; (2) step 3 — offline cache for `api_connection` (today it has none; only `screen_designer` persists locally), caching the last-fetched OpenAPI schema + discovered URL so the app can boot in a "last known backend" state without connectivity; (3) the AWS VM still needs `LLM_PROVIDER`/`OPENAI_API_KEY`/`GEMINI_API_KEY` added for the WEB voice assistant (DD180) — unrelated to this mobile feature, still pending from before.

**Update 2026-09-22 (voice-driven diagram assistant):** done and confirmed working end-to-end by the user against the live OpenAI API (see `CURRENT_STATE.md`, DD180), uncommitted — ask before committing. Next candidates: (1) deploy this to the AWS VM — the `.env` there needs `LLM_PROVIDER`/`OPENAI_API_KEY`/`OPENAI_MODEL` (or the Gemini equivalents) added, same as local; (2) the mobile AI feature (Gemini/Ollama on the phone) — the user's original other pending item, not started, and now that voice-on-web works this could reuse the same `apps/ai_assistant` endpoint instead of a separate mobile-only implementation; (3) surface which provider is active in the UI (currently silent — a wrong `.env` value fails with a generic error, no visible "using OpenAI/Gemini" indicator); (4) SSH-tunnel DB access for TablePlus, still not set up; (5) real SMTP config, still the console backend.

**Update 2026-09-22 (diagram canvas previews a many-to-many's join table):** done (see `CURRENT_STATE.md`, DD177), uncommitted — ask before committing. Next candidates: the same `/join-tables` endpoint could feed the mobile screen designer too, once it needs relationship/FK fields (see the mobile NEXT_STEPS entry below on filling `classAId` from a related record — this endpoint already knows which column is which FK).

**Update 2026-09-22 (mobile Field picker offers real attribute names):** done (see `CURRENT_STATE.md`, DD176), uncommitted — ask before committing. The APK is pushed to the phone but not yet confirmed run there; do that first if picking this up. Next candidates, in order: (1) a data grid widget, so `update`/`delete` can pick a record instead of typing its id, and so a `list` result renders as more than raw text; (2) let a Button on one class read a value from a Field bound to ANOTHER class (needed for relationship ids like `classAId` — `C`'s picker now shows that field exists, but there is still no way to fill it from a related Class A record); (3) mark required attributes in the picker (the schema already carries a `required` array, unused so far); (4) Modelia login + deployment selector, still not started.

**Update 2026-09-21 (edit a relationship on the canvas):** done (see `CURRENT_STATE.md`). Next candidates: edit the relationship kind and role names from the same dialog, and `RenameClass` (still the only command with no UI). A multiplicity with lower > upper is accepted by the backend and surfaces as an `INVALID_MULTIPLICITY` diagnostic (same as `AddRelationship`); only the dialog blocks it.

**Update 2026-09-21 (Flutter stage 1 `mobile-openapi-connect` done):** see `CURRENT_STATE.md`. Ask the user before committing (two work-unit commits were planned: A domain+parser+client, B controller+UI+wiring+Android+docs). Next candidates, in order: (1) a manual smoke against a real `/gen/<id>` deployment (start one from the web app, run the Flutter app with `--dart-define=GENERATED_API_URL=<its URL>`); (2) Modelia login + deployment selector, to replace typing a URL by hand; (3) a `mobile-ui.json` contract (labels, field order, widgets) and dynamic CRUD forms driven by it + the OpenAPI document. Optional cleanup: `mobile/lib/core/config/app_config.dart` still defaults the unrelated `API_BASE_URL` to `10.0.2.2:8000`; revisit once the Modelia login stage lands.

**Update 2026-09-21 (root environment consolidation; uncommitted):** on a VM, copy root `env.example` to root `.env`, fill the required secrets and domain, then run `docker compose -f docker-compose.prod.yml up -d --build`; never instruct or use `docker compose down`. Root `.env` is the sole real config (root `env.example` is its committed template); dev injects it only into backend/frontend and production interpolates it while retaining explicit service environments to avoid injecting secrets into Caddy/frontend. Obsolete templates are deleted (`backend/env.example`, `frontend/env.local.example`, `runner/env.example`, `deploy/env.prod.example`); dev and production Compose config checks passed (production used temporary shell-only `RUNNER_TOKEN=config-check` because the template intentionally leaves it blank). No services changed and commit remains pending user approval on `chore/unify-root-env`.
