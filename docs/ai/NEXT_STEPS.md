...
**Update 2026-09-21 (IN PROGRESS, do this first):** finish Flutter stage 1 `mobile-openapi-connect` following `openspec/changes/mobile-openapi-connect/tasks.md` and the resume checklist in `HANDOFF_LATEST.md`: get the Flutter SDK running (`C:\src\flutter`), confirm the RED tests, implement slice A then slice B with Strict TDD, `flutter analyze` + `flutter test`, Android manifests, docs, verify, archive. Then ask the user for authorization before any commit (two work-unit commits planned, `size:exception`). After stage 1, the ordered follow-ups are: Modelia login + deployment selector (replaces the manual URL), `mobile-ui.json` contract (labels, order, widgets), dynamic CRUD forms driven by OpenAPI + `mobile-ui.json`, and a manual smoke test against a real `/gen/<id>` deployment (task 8.2). Optional cleanup: `mobile/lib/core/config/app_config.dart` still defaults `API_BASE_URL` to `10.0.2.2:8000`; revisit when the Modelia login stage lands.
