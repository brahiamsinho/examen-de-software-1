...
**Update 2026-09-22 (mobile screen designer mixes several classes):** done (see `CURRENT_STATE.md`, DD175), uncommitted — ask before committing. The updated APK is pushed to the phone but the user has not yet confirmed it runs; do that first if picking this up. Next candidates, in order: (1) parse `components.schemas` in `openapi_parser.dart` so Field widgets can offer a real attribute list instead of a typed-by-hand name (would also let the designer warn about a class's required fields, e.g. `C`'s `classAId`, before a create fails); (2) a data grid widget, so `update`/`delete` can pick a record instead of typing its id, and so a `list` result renders as more than raw text; (3) let a Button on one class read a value from a Field bound to ANOTHER class (needed for relationship ids like `classAId`); (4) Modelia login + deployment selector, still not started.
