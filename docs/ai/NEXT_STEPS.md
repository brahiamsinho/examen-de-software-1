...
**Update 2026-09-22 (mobile screen designer):** done (see `CURRENT_STATE.md`, DD174), uncommitted — ask before committing. Next candidates, in order: (1) reinstall the updated APK on the user's phone and try it on a real device (push via `adb push` + tap-to-install in Files, same as the `mobile-openapi-connect` stage — HyperOS blocks a plain `flutter run`/`adb install`); (2) parse `components.schemas` in `openapi_parser.dart` so Field widgets can offer a real attribute list instead of a typed-by-hand name; (3) a data grid widget, so `update`/`delete` can pick a record instead of typing its id; (4) Modelia login + deployment selector, still not started.
