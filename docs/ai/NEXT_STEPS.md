...
**Update 2026-09-20 (newest):** `uml-generation-profile-authoring` is applied, verify pending, uncommitted (31/31 tasks, Strict TDD, backend 1107 -> 1184 passed, `apps/uml_commands` 86, `apps/uml_documents` 134, ~1208 authored lines, `size:exception` accepted; DD151-DD159). The last commit is `cdae44c feat(domain-manifest): emit declared generation profile`. **Next: `sdd-verify` for this change, then archive (merge the delta including both MODIFIED requirements and the rescoped persisted-layout scenario, touch up stale text), then commit (never `.pi/`).** After that, in order: (1) frontend change `uml-generation-profile-panel`: a 'Perfil de generación' Card in the sidebar with tri-state controls prefilled from `generation_metadata`, sending `SetGenerationProfile`; (2) filtering/search in the generated Spring API using `searchable` / `sortable` / `defaultSort`; (3) make `crud` restrict `operations[]` in the manifest and the generator (DD147 tech debt); (4) §37 item 12 remainder (inheritance DTOs/services/controllers, relation-navigation endpoints); (5) Flutter frontend (§37 item 17).
